...
def evaluate(env, agent, video, num_episodes, eval_mode, adapt=False, seed=0, domain_name='cartpole', task_name='swingup', algo='svea'):
	...
	for i in pbar:
		...
		episode_rewards.append(episode_reward)
		pbar.set_postfix({'current': '{:.5f}'.format(episode_reward),
                          'mean': '{:.5f}'.format(np.mean(episode_rewards)),
						  'std': '{:.5f}'.format(np.std(episode_rewards))
								})
		if i % 10==0:
			video.save(f'{i}.mp4')
...
def main(args):
	...
	np.save(f"{work_dir}/{args.eval_mode}_episode_rewards.npy",reward)
