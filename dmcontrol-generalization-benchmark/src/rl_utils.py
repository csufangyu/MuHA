...
def compute_guided_actor(obs, actor,encoder):
    model = ActorModelWrapper(actor=actor,encoder=encoder)
    gbp = GuidedBackprop(model)
    attribution = gbp.attribute(obs)
    return attribution

class ActorModelWrapper(torch.nn.Module):
    def __init__(self, encoder, actor):
        super(ActorModelWrapper, self).__init__()
        self.encoder = encoder
        self.actor = actor

    def forward(self, obs,std=1.0,is_train=True):
        obs=self.encoder(obs)
        _,mu=self.actor(obs,std,is_train=is_train)
        return mu.sum(dim=1)
    
class ActorFullModelWrapper(torch.nn.Module):
    def __init__(self, encoder, actor,critic,random=False):
        super(ActorFullModelWrapper, self).__init__()
        self.encoder = encoder
        self.actor = actor
        self.critic = critic
        self.random=random
    def forward(self, obs,std=1.0,is_train=True):
        ...
        _,mu=self.actor(obs,std,is_train=is_train)
        if self.random:
            action=torch.rand_like(mu)
        else:
            action=mu
        return self.critic(obs, action)[0]

def compute_guided_full_model(obs, actor,encoder,critic,random=False):
    model = ActorFullModelWrapper(actor=actor,encoder=encoder,critic=critic,random=random)
    gbp = GuidedBackprop(model)
    attribution = gbp.attribute(obs)
    return attribution
...
def compute_attribution_mask_random(obs_grad, quantile_list):
    mask = []
    for i in [0, 3, 6]:
        quantile= random.choice(quantile_list)
        attributions = obs_grad[:, i : i + 3].abs().max(dim=1)[0]
        q = torch.quantile(attributions.flatten(1), quantile, 1)
        mask.append((attributions >= q[:, None, None]).unsqueeze(1).repeat(1, 3, 1, 1))
    return torch.cat(mask, dim=1)
