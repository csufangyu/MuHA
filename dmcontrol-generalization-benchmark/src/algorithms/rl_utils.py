...
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, obs):
        if self.action is None:
            ...
            obs = self.encoder(obs)
            obs = self.model(obs,self.action)
        return obs[0]

def compute_guided_backprop(obs, action,encoder, model):
    model = ModelWrapper(encoder, model, action=action)
...
def compute_attribution(encoder,model, obs, action=None,method="guided_backprop"):
    if method == "guided_backprop":
        return compute_guided_backprop(obs, action,encoder, model)
